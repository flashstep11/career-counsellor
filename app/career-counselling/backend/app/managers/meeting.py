...
class MeetingManager:
    ...
    async def get_available_slots(self, expert_id: str, date_str: str) -> List[Dict]:
        ...
        if not expert.get("available", True):
            return []
    ...
    async def book_meeting(
        ...
        if not expert.get("available", True):
            raise ValueError("This expert is not accepting new bookings right now")
        ...
        for m in meetings:
            ...
            if expert:
                ...
                if user_doc:
                    ...
                    m["expertAvatar"] = user_doc.get("profile_picture_url") or user_doc.get("avatar") or ""
        return meetings
    ...
    async def get_all_meetings_for_user(self, user_id: str, status: Optional[str] = None) -> List[Dict]:
        ...
        for m in meetings:
            ...
            feedback_exists = await self.db.meeting_feedbacks.find_one({"meetingId": mid})
            m["hasFeedback"] = feedback_exists is not None
    ...
    async def send_due_reminders(self) -> int:
        """
        Check for SCHEDULED meetings starting in <= 11 minutes 
        that do not have reminderSent == True, and send notifications.
        """
        ...
        now = now_app_naive()
        upper_limit = now + timedelta(minutes=11)
        lower_limit = now - timedelta(minutes=5)

        query = {
            "status": {"$in": [MeetingStatus.SCHEDULED, "scheduled"]},
            "startTime": {"$gte": lower_limit, "$lte": upper_limit},
            "reminderSent": {"$ne": True}
        }
        
        meetings = await self.collection.find(query).to_list(None)
        if not meetings:
            return 0
        ...
        for meeting in meetings:
            ...
            student_user_id = meeting["userId"]
            expert_user_id = expert["userId"]
            current_time = datetime.utcnow()
            
            try:
                # To student
                await notification_manager.create_notification(Notification(
                    targetUserId=student_user_id,
                    sourceUserId=expert_user_id,
                    type=NotificationType.MEETING_REMINDER,
                    content="Your meeting is starting in less than 10 minutes!",
                    referenceId=meeting_id,
                ...
                # To expert
                await notification_manager.create_notification(Notification(
                    targetUserId=expert_user_id,
                    sourceUserId=student_user_id,
                    type=NotificationType.MEETING_REMINDER,
                    content="Your meeting is starting in less than 10 minutes!",
                    referenceId=meeting_id,
                    referenceType="meeting",
