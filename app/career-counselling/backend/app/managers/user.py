...
class UserManager:
    ...
    async def follow_user(
        ...
        try:
            ...
            # Enforce that the target user is an expert unless explicitly allowed
            if not target.isExpert and not allow_non_expert:
                return False
    ...
    async def get_followers(self, user_id: str, skip: int = 0, limit: int = 50) -> List[User]:
        """
        ...
        Returns:
            List[User]: List of follower users
        """
        try:
            ...
            follower_ids = user.followers if isinstance(user.followers, list) else []
            if not follower_ids:
                return []

            paged_ids = follower_ids[skip:skip + limit]
            object_ids = []
            for follower_id in paged_ids:
                try:
                    object_ids.append(ObjectId(follower_id))
                except Exception:
                    continue
            ...
            followers = await self.db.users.find({"_id": {"$in": object_ids}}).to_list(None)
            followers_by_id = {str(f["_id"]): f for f in followers}

            ordered_followers = []
            for follower_id in paged_ids:
                follower = followers_by_id.get(follower_id)
                if not follower:
                    continue
                follower["id"] = str(follower["_id"])
                follower["_id"] = str(follower["_id"])
                follower.pop("hashedPassword", None)
                follower.pop("password", None)
                follower["email"] = None
                follower["mobileNo"] = None
                ordered_followers.append(User(**follower))

            return ordered_followers
        except Exception as e:
            print(f"Error retrieving followers: {e}")
            return []

    async def get_following(self, user_id: str, skip: int = 0, limit: int = 50) -> List[User]:
        """Get a paginated list of users that a user is following."""
        try:
            user = await self.get_user(user_id)
            if not user:
                return []

            following_ids = user.following if isinstance(user.following, list) else []
            if not following_ids:
                return []

            paged_ids = following_ids[skip:skip + limit]
            object_ids = []
            for followed_id in paged_ids:
                try:
                    object_ids.append(ObjectId(followed_id))
                except Exception:
                    continue
            ...
            followed_users = await self.db.users.find({"_id": {"$in": object_ids}}).to_list(None)
            followed_by_id = {str(u["_id"]): u for u in followed_users}

            ordered_following = []
            for followed_id in paged_ids:
                followed = followed_by_id.get(followed_id)
                if not followed:
                    continue
                followed["id"] = str(followed["_id"])
                followed["_id"] = str(followed["_id"])
                followed.pop("hashedPassword", None)
                followed.pop("password", None)
                followed["email"] = None
                followed["mobileNo"] = None
                ordered_following.append(User(**followed))

            return ordered_following
        except Exception as e:
            print(f"Error retrieving following: {e}")
            return []
    ...
    async def get_dashboard_stats(self) -> Dict:
        ...
        total_connections = await self.db.connections.count_documents({"status": "accepted"})
        total_followers = await self.db.follows.count_documents({"status": "accepted"})
