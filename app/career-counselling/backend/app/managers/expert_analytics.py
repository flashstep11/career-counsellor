...
class ExpertAnalyticsManager:
    def __init__(self):
        ...
        self.profile_view_events_collection = self.db.expert_profile_view_events
    ...
    async def get_expert_analytics(self, expert_id: str) -> Dict:
        ...
        try:
            ...
            # Get content counts and views using efficient MongoDB aggregation
            # Videos: count and sum views
            videos_count_future = self.videos_collection.count_documents({"userId": user_id})
            videos_agg_future = self.videos_collection.aggregate([
                {"$match": {"userId": user_id}},
                {"$group": {"_id": None, "totalViews": {"$sum": "$views"}}}
            ]).to_list(1)

            # Blogs: count and sum views (supports both userId and legacy userID)
            blogs_query = {
                "$or": [
                    {"userId": user_id},
                    {"userID": user_id},
            ...
            blogs_count_future = self.blogs_collection.count_documents(blogs_query)
            blogs_agg_future = self.blogs_collection.aggregate([
                {"$match": blogs_query},
                {"$group": {"_id": None, "totalViews": {"$sum": "$views"}}}
            ]).to_list(1)

            # Posts: count and sum views
            posts_count_future = self.posts_collection.count_documents({"expertId": expert_id})
            posts_agg_future = self.posts_collection.aggregate([
            ...
            video_views = videos_agg[0]["totalViews"] if videos_agg else 0
            blog_reads = blogs_agg[0]["totalViews"] if blogs_agg else 0
            post_views = posts_agg[0]["totalViews"] if posts_agg else 0
            ...
            # Calculate ratings distribution with a single aggregation query
            ratings_buckets = await self.ratings_collection.aggregate([
                {"$match": {"expertId": expert_id}},
                {"$group": {"_id": "$rating", "count": {"$sum": 1}}}
            ]).to_list(10)
            ratings_map = {
                int(item.get("_id", 0)): item.get("count", 0)
                for item in ratings_buckets
                if item.get("_id") is not None
            }
            ratings_distribution = [
                {"rating": i, "count": ratings_map.get(i, 0)}
                for i in range(1, 6)
            ]
    ...
    async def track_profile_view(
        ...
        Args:
            ...
            viewer_key (Optional[str]): Unique key for viewer (user or anon fingerprint)
            dedupe_hours (int): Ignore duplicate views from same viewer within this window
        ...
        try:
            ...
            # Prevent rapid/infinite increments from same viewer (refresh abuse)
            if viewer_key:
                window_start = now - datetime.timedelta(hours=dedupe_hours)
                existing = await self.profile_view_events_collection.find_one({
                    "expertId": expert_id,
                    "viewerKey": viewer_key,
                    "viewedAt": {"$gte": window_start},
                })
                if existing:
                    return True
            ...
            if viewer_key:
                await self.profile_view_events_collection.insert_one({
                    "expertId": expert_id,
                    "viewerKey": viewer_key,
                    "viewedAt": now,
                })
