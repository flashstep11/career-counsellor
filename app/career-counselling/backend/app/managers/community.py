...
class CommunityManager:
        ...
        try:
            ...
            result = await self.collection.update_one(
                {**query, "members": user_id},
                {
                    ...
                    "$unset": {f"community_roles.{user_id}": ""},
                    "$inc": {"memberCount": -1},
                    "$set": {"updatedAt": datetime.utcnow()},
                },
            )
            ...
            # If members remain but there are no moderators, auto-promote one member
            doc = await self.collection.find_one(
                query,
                {"members": 1, "community_roles": 1, "createdBy": 1},
            )
            if not doc:
                return True

            members: List[str] = doc.get("members", []) or []
            if not members:
                # Community is now empty. Assign a random admin as moderator so the
                # community isn't left without any members/moderators.
                try:
                    admins = await self.db.users.aggregate(
                        [
                            {"$match": {"isAdmin": True}},
                            {"$sample": {"size": 1}},
                            {"$project": {"_id": 1}},
                ...
                await self.collection.update_one(
                    query,
                    {
                        ...
                        "$set": {
                            "createdBy": admin_id,
                            f"community_roles.{admin_id}": "moderator",
            ...
            active_mods = {
                ...
                if role == "moderator" and uid in members
            }
            created_by = doc.get("createdBy")
            if created_by in members:
                active_mods.add(created_by)

            if active_mods:
                return True

            def _oid_key(uid: str):
                try:
                    return ObjectId(uid)
                except Exception:
                    return None

            with_oid = [(m, _oid_key(m)) for m in members]
            with_oid.sort(key=lambda x: (x[1] is None, x[1] or ObjectId("ffffffffffffffffffffffff"), x[0]))
            new_mod_id = with_oid[0][0]

            update: dict = {
                "$set": {f"community_roles.{new_mod_id}": "moderator", "updatedAt": datetime.utcnow()},
            }
            if created_by not in members:
                update["$set"]["createdBy"] = new_mod_id
    ...
    async def _to_response(self, doc: dict, requesting_user_id: Optional[str]) -> CommunityResponse:
        ...
        is_joined = requesting_user_id in doc.get("members", []) if requesting_user_id else False
        doc["isJoined"] = is_joined
    ...
    async def is_moderator(self, community_id: str, user_id: str) -> bool:
        """Return True if user is an active member and is createdBy or has moderator role."""
        try:
            doc = await self.collection.find_one(
                {"_id": ObjectId(community_id)},
                {"createdBy": 1, "community_roles": 1, "members": 1},
            )
            if not doc:
                return False
