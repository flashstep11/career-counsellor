...
def _get_hs256_secret() -> str:
    """
    Ensure a minimum 32-byte secret for HS256.
    If the configured secret is shorter, derive a 32+ byte secret via SHA-256.
    """
    secret = settings.SECRET_KEY or ""
    if len(secret) >= 32:
        return secret
    logging.getLogger(__name__).warning(
        "SECRET_KEY length is below 32 bytes; deriving HS256 secret via SHA-256"
    )
...
async def get_token(email: str) -> str:
    ...
    token = jwt.encode(payload, _get_hs256_secret(), algorithm=settings.ALGORITHM)
    return token
...
async def verify_token(token: str) -> Optional[Dict[str, str]]:
    ...
    """
    try:
        payload = jwt.decode(token, _get_hs256_secret(), algorithms=[settings.ALGORITHM])
