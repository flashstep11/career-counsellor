...
def user_room(user_id: str) -> str:
    """Return the personal socket room for a user."""
    return str(user_id)


def chat_room(user_a: str, user_b: str) -> str:
    """Return the shared socket room for a two-person chat."""
    return f"chat:{':'.join(sorted([str(user_a), str(user_b)]))}"


async def _emit_chat_error(sid: str, reason: str) -> None:
    await sio.emit("chat:error", {"reason": reason}, to=sid)
    await sio.emit("chat_error", {"reason": reason}, to=sid)


def _chat_error_message(error_code: str) -> str:
    messages = {
        "not_connected": "You can only chat with accepted connections.",
        "self_chat": "You cannot start a chat with yourself.",
        "empty_message": "Message cannot be empty.",
        "message_too_long": "Message is too long.",
    }
    return messages.get(error_code, "Message could not be sent.")


@sio.event
async def join_chat(sid, data):
    """Join a private chat room with another accepted connection."""
    from app.managers.chat import ChatManager

    session = await sio.get_session(sid)
    user_id = session.get("user_id") if session else None
    target_user_id = data.get("target_user_id") or data.get("targetUserId") if data else None
    if not user_id or not target_user_id:
        await _emit_chat_error(sid, "Could not open this chat.")
        return

    manager = ChatManager()
    try:
        await manager._ensure_connected(str(user_id), str(target_user_id))
    except ValueError as exc:
        await _emit_chat_error(sid, _chat_error_message(str(exc)))
        return

    await sio.enter_room(sid, chat_room(user_id, target_user_id))


@sio.event
async def send_message(sid, data):
    """Persist and broadcast a direct message to an accepted connection."""
    from app.managers.chat import ChatManager

    session = await sio.get_session(sid)
    user_id = session.get("user_id") if session else None
    recipient_id = data.get("receiver_id") or data.get("recipientId") if data else None
    content = data.get("content") if data else None

    if not user_id or not recipient_id:
        await _emit_chat_error(sid, "Message could not be sent.")
        return

    manager = ChatManager()
    try:
        message = await manager.create_message(str(user_id), str(recipient_id), str(content or ""))
    except ValueError as exc:
        await _emit_chat_error(sid, _chat_error_message(str(exc)))
        return

    payload = message.model_dump(mode="json")
    room = chat_room(user_id, recipient_id)
    ...
    await sio.emit("chat:message", payload, room=user_room(user_id))
    await sio.emit("chat:message", payload, room=user_room(recipient_id))
