...
async def get_pending_moderator_applications_for_community(
    ...
    try:
        ...
        if not await application_manager.community_manager.is_moderator(community.communityId, current_user.id):
            raise HTTPException(status_code=403, detail="You must still be an active moderator of this community")
