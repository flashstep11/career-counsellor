...
async def accept_connection(
    ...
    # Auto-follow on successful connection (bidirectional)
    try:
        await user_manager.follow_user(conn.requester_id, conn.target_id, allow_non_expert=True, notify=False)
        await user_manager.follow_user(conn.target_id, conn.requester_id, allow_non_expert=True, notify=False)
    except Exception:
        # Connection acceptance should not fail if follow sync fails
        pass
