...
@router.get("/videos/my-videos", response_model=List[Dict])
async def get_my_videos(user_data: dict = Depends(require_user)):
    """
    Get videos created by the authenticated user.
    Returns a frontend-friendly shape used by VideoManagement.
    """
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to fetch your videos",
        )
...
async def get_video(video_id: str, user_data: dict = Depends(require_user)):
    """
