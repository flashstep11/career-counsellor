...
async def create_comment(comment_data: CommentCreate, user_data: dict = Depends(get_current_user)):
    ...
    # If this is a reply, check nesting depth
    if comment_data.parent_id:
        depth = 1
        parent_id = comment_data.parent_id
        while parent_id:
            parent_doc = await db.comments.find_one({"_id": ObjectId(parent_id)})
            if not parent_doc:
                break
            parent_id = parent_doc.get("parent_id")
            depth += 1
        if depth > MAX_NESTING_DEPTH:
            raise HTTPException(status_code=400, detail=f"Maximum reply nesting depth of {MAX_NESTING_DEPTH} reached.")
                ...
                if parent_author_email:
                    ...
                    if parent_user:
                        ...
                        await notification_manager.create_reply_notification(
                            ...
                            comment_id=str(result.commentID) if getattr(result, "commentID", None) else "",
                            post_id=post_id,
                        )
