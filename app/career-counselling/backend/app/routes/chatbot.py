...
async def enhance_content(request: EnhanceContentRequest, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        logger.exception("Error enhancing content")
        # Graceful fallback to avoid blocking post creation
        return {"enhanced_content": request.content}
...
async def send_system_prompt(chat_message: ChatMessage, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        logger.exception("Error sending system prompt")
        raise HTTPException(
