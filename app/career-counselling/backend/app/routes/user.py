...
@router.get("/users/me/followers", response_model=List[User])
async def list_my_followers(
    ...
    """
    current_user = await user_manager.get_user_by_email(user_data.get("email", ""))
    if not current_user and user_data.get("id"):
        current_user = await user_manager.get_user(user_data["id"])
    ...
    return await user_manager.get_followers(current_user.id, skip=skip, limit=limit)


@router.get("/users/me/following", response_model=List[User])
async def list_my_following(
    ...
    return await user_manager.get_following(current_user.id, skip=skip, limit=limit)
...
async def get_user_posts(user_id: str, page: int = 1, limit: int = 10):
    """
    Get posts created by a specific user with pagination support.
    ...
    Returns a paginated response with posts and metadata.
    """
    try:
        ...
        total = await posts_collection.count_documents({"authorId": user_id})
...
@router.get("/users/followers", response_model=List[User])
async def list_followers_legacy(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(20, ge=1, le=100, description="Maximum number of records to return"),
    user_data: dict = Depends(require_user),
):
    ...
    Prefer /users/me/followers to avoid route conflicts.
    """
    current_user = await user_manager.get_user_by_email(user_data.get("email", ""))
    if not current_user and user_data.get("id"):
        current_user = await user_manager.get_user(user_data["id"])
