  ...
  useEffect(() => {
    const fetchExpertVideos = async () => {
      ...
      try {
        ...
        const summary = axios.isAxiosError(err)
          ? `${err.config?.method?.toUpperCase() || "REQUEST"} ${err.config?.url || ""} failed with ${err.response?.status || "unknown status"}`
          : "Expert videos request failed";
