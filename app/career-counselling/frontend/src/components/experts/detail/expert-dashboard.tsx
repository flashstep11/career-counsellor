...
export default function ExpertDashboard({
  ...
  const [socialLinksDraft, setSocialLinksDraft] = useState<SocialLinkEntry[]>(
  ...
  const meetingSettingsSaveInFlightRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const fetchEarningsData = async () => {
      try {
        ...
        // earnings fetch failure is non-fatal and should not block dashboard render
      }
    };
  ...
  const handleSaveMeetingSettings = async () => {
    if (meetingSettingsSaveInFlightRef.current) return;

    meetingSettingsSaveInFlightRef.current = true;
    ...
    try {
      ...
      meetingSettingsSaveInFlightRef.current = false;
  ...
  const handleSaveProfile = async () => {
    setIsLoading(true);
    try {
      const sanitizedSocialLinks = socialLinksDraft.reduce<Record<string, string>>(
        (acc, item) => {
  ...
  const addSocialLink = () => {
    setSocialLinksDraft((prev) => [...prev, { platform: "", url: "" }]);
  };

  const removeSocialLink = (index: number) => {
    setSocialLinksDraft((prev) => prev.filter((_, i) => i !== index));
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-1 md:col-span-2 space-y-3">
                ...
                {socialLinksDraft.length === 0 && (
                  <p className="text-xs text-gray-500">
                    Add links like LinkedIn, Twitter, YouTube, portfolio, or any custom platform.
                  </p>
                )}

                <div className="space-y-2">
                  {socialLinksDraft.map((link, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-[160px_1fr_auto] gap-2">
                      <Input
                      ...
                      {earningsData.sessions.length > 10 && (
                        <div className="px-4 py-3 bg-gray-50 flex items-center justify-between gap-3">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => (window.location.href = "/meetings?tab=past&status=completed")}
                          >
            ...
            <CardContent className="pt-4">
              ...
              >
                ...
                <TabsContent value="blogs" className="space-y-4">
                  {activeContentTab === "blogs" && <BlogManagement />}
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-400">
                  Students pay this per {meetingSettings.sessionDurationMinutes}-minute session.
                </p>
              </div>
