...
export default function FollowButton({ targetUserId, className = "", label = "Follow", followingLabel = "Unfollow" }: FollowButtonProps) {
  ...
  const summarizeAxiosError = (error: unknown) => {
    if (axios.isAxiosError(error)) {
      return `${error.config?.method?.toUpperCase() || "REQUEST"} ${error.config?.url || ""} failed with ${error.response?.status || "unknown status"}`;
    }
    return "Follow status request failed";
  };
  ...
  useEffect(() => {
    ...
    const checkFollowingStatus = async () => {
      ...
      try {
        ...
        console.warn(summarizeAxiosError(error));
      } finally {
        setIsLoading(false);
      }
    };
