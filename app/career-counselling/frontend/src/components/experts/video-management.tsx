...
export function VideoManagement({ expertId }: VideoManagementProps) {
  ...
  const fetchVideos = async () => {
    try {
      ...
      if (!response.ok) {
        setVideos([]);
        toast.error("Could not load videos right now");
        return;
      }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1 max-w-2xl leading-relaxed">
            Upload and organize your educational videos. Mark one as your{" "}
        ...
        <Card className="p-10">
          <div className="text-center max-w-md mx-auto">
            ...
            <p className="text-sm text-gray-600 mb-5 leading-relaxed">
              Start sharing your expertise by uploading your first video.
            </p>
            <Button onClick={() => handleOpenDialog()} className="gap-2 min-w-44">
              <Plus className="h-4 w-4" />
              Upload First Video
            </Button>
          </div>
