...
const ExpertCard = ({ expert }: { expert: Expert }) => {
  ...
  const credentials = userDetails.credentials || [];
  const verifiedByCredential = credentials.some(
    (c) => (c || "").toLowerCase() === "verified"
  );
  // Fall back to heuristic when older data doesn't include credentials.
  ...
  const isNewExpert = (expert.rating ?? 0) <= 0;
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                ...
                {expert.rating !== undefined && (
                  isNewExpert ? (
                    <div className="bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-1 rounded-full">
                      New Mentor
                    </div>
                  ) : (
