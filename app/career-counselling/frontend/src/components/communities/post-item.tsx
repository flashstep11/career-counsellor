...
import { Heart, MessageSquare, Eye, Clock, Pin, Trash2, Flag, BadgeCheck, MoreVertical, Shield, Settings2, CheckCircle } from "lucide-react";
...
export default function PostItem({
    ...
    const canReport = isAuthenticated && communityId && !isAuthor;
    ...
    const isVerified = credentials.some((c) => (c || "").toLowerCase() === "verified");
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-3 text-xs text-gray-400">
                ...
                {isVerified && (
                    <span title="Verified" className="inline-flex items-center">
                        <CheckCircle className="h-3.5 w-3.5 fill-blue-600 text-white" />
                    </span>
                )}
                {/* Credential badges */}
            ...
            <Link href={`/posts/${post.postId}`}>
                <h3 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors mb-1.5 line-clamp-2 break-all">
                    {post.title}
                </h3>
                <p className="text-sm text-gray-500 line-clamp-2 mb-3 break-all">{post.content}</p>
            </Link>
            ...
            {post.media && post.media.length > 0 && (
                <div className={`grid gap-2 mb-3 ${post.media.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
                    {post.media.map((item, i) => (
                        <div key={i} className={`rounded-xl overflow-hidden border border-gray-100 bg-gray-50 flex items-center justify-center ${(post.media?.length || 0) > 1 ? "h-48" : ""}`}>
                            {item.type === "image" ? (
                                <img src={item.url} alt="" className={`w-full ${(post.media?.length || 0) === 1 ? "max-h-[600px] h-auto object-contain" : "h-full object-cover"}`} loading="lazy" />
                            ) : (
                                <video src={item.url} controls className={`w-full ${(post.media?.length || 0) === 1 ? "max-h-[600px] h-auto object-contain" : "h-full object-cover"}`} preload="metadata" />
                            )}
                        </div>
                    ))}
                </div>
