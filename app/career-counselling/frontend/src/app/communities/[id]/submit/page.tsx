...
export default function SubmitPostPage() {
    ...
    const handleEnhanceContent = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                let detail = `Failed to enhance content (status ${response.status})`;
                try {
                ...
                if (/GEMINI_API_KEY/i.test(detail)) {
                    setEnhanceUnavailable(true);
                }
                if (response.status === 401) {
                    detail = "Your session expired. Please log in again to use Enhance.";
                }
            ...
            const message = err instanceof Error ? err.message : "Failed to enhance content. Please try again.";
            setError(message);
            toast.error(message);
        } finally {
            setEnhancingContent(false);
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1.5">
                                ...
                                <Button
                                    ...
                                    disabled={!form.content.trim() || enhancingContent || enhanceUnavailable}
                                    variant="outline"
                                    size="sm"
