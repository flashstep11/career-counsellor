...
import {
  ...
  GraduationCap, Star, Clock, Users, CalendarCheck, MessageCircle,CheckCircle,
} from "lucide-react";
import {
  Dialog,
  DialogContent,
...
export default function UserProfilePage() {
  ...
  const expertInitials = expert
    ...
    : "";
  const verifiedByCredential = (user.credentials ?? []).some(
    (c) => (c || "").toLowerCase() === "verified"
  );
  const heuristicExpertVerified = expert
    ? expert.rating >= 4.0 || expert.studentsGuided > 10
    : false;
  const isVerified = verifiedByCredential || heuristicExpertVerified;
