...
export default function CollegesPage() {
  ...
  const handlePredict = async () => {
    ...
    const token = localStorage.getItem("token");
    if (!token) {
      alert("Please sign in to use the predictor");
      return;
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] overflow-hidden -mx-6 px-6">
      ...
      <Dialog open={showMissingRankModal} onOpenChange={setShowMissingRankModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Enter Exam Rank Required</DialogTitle>
            <DialogDescription>
              Please enter your rank for at least one exam to get predicted college recommendations.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
              <Button onClick={() => setShowMissingRankModal(false)} autoFocus>
                OK
              </Button>
            </DialogClose>
          </DialogFooter>
