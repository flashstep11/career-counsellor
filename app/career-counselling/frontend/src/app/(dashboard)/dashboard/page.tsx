...
function MyFeedSection() {
  ...
  const [videos, setVideos] = useState<VideoType[]>([]);
  const [loading, setLoading] = useState(true);
  const [openBlogMenuId, setOpenBlogMenuId] = useState<string | null>(null);
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {feed.map((entry) => {
            if (entry.kind === "post") {
              return (
                <PostItem key={entry.data.postId} post={entry.data} showCommunity communityId={entry.data.communityId} />
              );
            }
            if (entry.kind === "blog") {
              ...
              return (
                <Link key={blog.blogID} href={`/blogs/${blog.blogID}`}>
                  <div className="group relative bg-white border border-gray-100 hover:border-emerald-200 rounded-2xl shadow-sm hover:shadow-md transition-all p-5">
                    {canReport && (
                      <div className="absolute top-3 right-3 z-10">
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setOpenBlogMenuId((v) => (v === blog.blogID ? null : blog.blogID));
                          }}
