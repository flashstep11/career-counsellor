...
function MeetingsDashboardContent() {
    ...
    const requestedTab = searchParams.get("tab") === "past" ? "past" : "upcoming";
    const requestedStatus = (searchParams.get("status") || "").toLowerCase();
    const [activeTab, setActiveTab] = useState<"upcoming" | "past">(requestedTab);
    ...
    const visiblePastMeetings =
        requestedStatus === "completed"
            ? pastMeetings.filter((meeting) =>
    ...
    const MeetingCard = ({ meeting, isPast }: { meeting: Meeting, isPast: boolean }) => {
        ...
        const expertName = meeting.expertName?.trim() || "Expert";
                        ...
                        <div className="flex-1 space-y-4">
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <Link
                                        href={`/experts/${meeting.expertId}`}
                                        className="inline-flex items-center gap-3 rounded-md text-sm text-gray-600 transition-colors hover:text-indigo-600"
                                    >
                                        <Avatar className="h-9 w-9">
                                            <AvatarImage src={meeting.expertAvatar || undefined} alt={expertName} />
                        ...
                        {(!isPast || !user?.isExpert) && (
                            <div className="flex flex-col sm:items-end justify-center gap-2 border-t sm:border-t-0 sm:border-l border-gray-100 pt-4 sm:pt-0 sm:pl-4 min-w-[160px]">
                                {isPast ? (
                                    !user?.isExpert && (
                                        <>
                                            <Button className="w-full mb-2" variant="outline" onClick={() => router.push(`/experts/${meeting.expertId}`)}>
                                                Book Again
                                            </Button>

                                            {meeting.status?.toLowerCase() === 'completed' && !meeting.hasFeedback && (
                                                <Button
                                                    className="w-full gap-2 bg-yellow-500 hover:bg-yellow-600 text-white"
                                                    onClick={() => handleOpenFeedback(meeting._id || meeting.id!)}
                                                >
                                                    <Star className="h-4 w-4" /> Rate Session
                                                </Button>
                                            )}
                                            {meeting.status?.toLowerCase() === 'completed' && meeting.hasFeedback && (
                                                <Button className="w-full gap-2 text-green-600 bg-green-50 hover:bg-green-50 cursor-default" variant="secondary" onClick={() => { }}>
                                                    <Star className="h-4 w-4 fill-green-600" /> Rated
                                                </Button>
                                            )}
                                        </>
                                    )
            ...
            <div className="min-h-screen bg-gray-50/50 py-10 px-4 sm:px-6 lg:px-8">
                    ...
                    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "upcoming" | "past")} className="w-full">
                        <TabsList className="grid w-full grid-cols-2 max-w-[400px] mb-8 bg-white shadow-sm border border-gray-100 p-1 h-12 rounded-xl">
                            ...
                            <TabsTrigger value="past" className="rounded-lg text-base font-medium data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900">
                                Past ({visiblePastMeetings.length})
                            </TabsTrigger>
                        </TabsList>
                ...
                {selectedMeetingId && (
                    <FeedbackModal
                        ...
                        onSuccess={() => {
                            // fetchMeetings to update the UI button to "Rated"
                            fetchMeetings();
                        }}
                    />
                )}
